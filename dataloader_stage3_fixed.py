...
class Dataloader(data.Dataset):
    def __init__(self, train = True, data_reader = None, labels = None, protein_reader = None):
        ...
        self.data_reader = self.data_reader.item()
...
class PrepareDataloader():
    def __init__(self, config):
        ...
        test_atac_loaders = []
        self.num_of_atac = 0
        #print(len(config.atac_paths))
        #print(len(config.atac_protein_paths))
        if len(config.atac_paths) == len(config.atac_protein_paths):
            ...
            for i, atac_path in enumerate(config.atac_paths):  
                ...
                #data_reader=data_reader.shape[0]
                #data_reader=data_reader.item()
                #data_reader=data_reader[0:200]
                #print(data_reader)
                #data_reader = npz_reader("sce_10xPBMC_atac_skip_200.npy") 
                #print(data_reader.shape[1])
                #print(len(labels))
                #print(data_reader)
                #print(labels)
                # train loader         
                #print(data_reader.item().shape())  
                #data_reader=data_reader.item()    
                #print(data_reader) 
                #print(data_reader)
                trainset = Dataloader(True, data_reader, labels)
